...
version: 5.3.0-mandatory-live-tail-rich-input
---

# Telegram Complete-Response Mirror + Mandatory Live Tail
...
6. continue bounded polling until a follow-up arrives or the active window expires;
7. when a follow-up arrives, consume it immediately, verify the `✅` pickup acknowledgement, process it, answer, mirror the new answer, and start a fresh 10-minute tail;
8. if no reply arrives, end naturally.
...
2. ChatGPT keeps polling after that response instead of immediately ending the turn;
...
A real v5.3 live-tail text test succeeded when a user-initiated Telegram reply was detected during the active tail and the ChatGPT pickup acknowledgement was returned through the fast acknowledgement path.
...
`Answer -> Telegram -> verified ID -> live 10-minute wait -> reply -> consume -> ✅ -> understand -> answer -> restart clock.`
