...
def frame_hist(bgr):
    ...
    h=cv2.calcHist([hsv],[0,1],None,[24,24],[0,180,0,256]);cv2.normalize(h,h);return h

def group_times(ts,gap=.35):
    if not ts:return []
    groups=[[ts[0]]]
    for t in ts[1:]:
        if t-groups[-1][-1]<=gap:groups[-1].append(t)
        else:groups.append([t])
    return [round(float(np.mean(g)),3) for g in groups]
...
def main():
    ...
    while True:
        ...
        small=cv2.resize(bgr,(160,90),interpolation=cv2.INTER_AREA);g=cv2.cvtColor(small,cv2.COLOR_BGR2GRAY);hist=frame_hist(small);t=idx/max(fps,1)
        if prev_g is not None:
            mad=float(np.mean(cv2.absdiff(prev_g,g)));corr=float(cv2.compareHist(prev_hist,hist,cv2.HISTCMP_CORREL));samples.append((t,mad,corr))
            if mad<0.7:
                if freeze_start is None:freeze_start=t-(step/max(fps,1))
